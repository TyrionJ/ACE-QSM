...
"""
Perform 3D tensor Discrete Wavelet Transform (DWT) and Inverse Discrete Wavelet Transform (IDWT).
The implementation does not account for boundary extension.
Accurate reconstruction is only achievable when the dimensions of the input tensor along all three axes are even,
and the reconstruction filter bank's low-frequency component has a length of 2.
Otherwise, reconstruction errors may occur at the boundaries.
"""
